...
Player::Player(Map *base):Character(),QGraphicsPixmapItem()
{
    setLvl(1);
    setExp(0);
    setExpMx(100);
    ...
    connect(timer, SIGNAL(timeout()), this, SLOT(refreshStatCh()));
    ...
    emit playLvlUpdate(getLvl(), getHpMx(), getExpMx()); //initialize statchild to player
}
...
bool Player::levelup(int exp){
    ...
    if(flag){
        ...
        setCrit(getCrit()+1);
        setExpMx(pExpMx+100);
        emit playLvlUpdate(getLvl(), getHpMx(), getExpMx()); //update statchild on levelup
    }
    return flag;
}
...
void Player::setCharge(int amount)
{
    pCharge = amount;
}
...
{
    //playtostat arguments: (hp, xp, weapon charge)
    emit playToStat(getHp(),getExp(),getCharge());
}
...
void Player::checkCollision(){
    ...
    for (int i = 0, n = colliding_items.size(); i < n; ++i){
        ...
        if(typeid(*(colliding_items[i])) == typeid(Bullet)){
            ...
            //THE TWO LINES ABOVE WERE CAUSING
            //A GAME BREAKING BUG ON COLLISION
            //WITH THE PLAYER
