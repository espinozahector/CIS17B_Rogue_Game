...
PJock::PJock(Map *base): QGraphicsPixmapItem()
{
    //set graphic
    playerx = 4;
    playery = 4;
...
void PJock::keyReleaseEvent(QKeyEvent *event)
{
    if (event->key() == Qt::Key_A)
    {
        isMovingLeft = false;
...
void PJock::nextFrame(char direction)
{
    switch (direction)
    {
        case 'l': {
...
void PJock::restartFrame()
{
    spriteCoordLeftRight = 0;
    spriteCoordUpDown = 0;
}

void PJock::trapDoor(int y,int x)
{
