...
Map::Map() : Cols() //Default Constructor
{
    numRoom = 10;
}
Map::Map(int newRow, int newCols) : Cols(newRow,newCols) //Constructor with two parameters
{
    numRoom = 10;
    setStrt(5,5); // sets the start of the map
    buildMap();   // Build map function
}
Map::~Map() // destructor
{
...
void Map::findEnd() //used to find the farthest location from the center
{
    ...
    for(int i = 0; i < numRoom; i++)
    {
        ...
        if(b<0) b*=-1;
        temp = a + b;
//        temp = (((RoomX[i]-x)*(RoomX[i]-x))+((RoomY[i]-y)*(RoomY[i]-y)));
//        temp = sqrt(temp);
        if(temp >= length)
        {
            length = temp;
